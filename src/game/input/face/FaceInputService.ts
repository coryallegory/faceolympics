...
export class FaceInputService {
  ...
  private async loadLandmarker(): Promise<void> {
    ...
    this.debugFrame = { ...this.debugFrame, status: 'loading', message: 'Loading MediaPipe face tracker…', updatedAt: Date.now() };
    try {
      ...
      this.debugFrame = { landmarks: [], blendshapes: {}, updatedAt: Date.now(), status: 'error', message: error instanceof Error ? error.message : 'Face tracker failed to load.' };
    } finally {
