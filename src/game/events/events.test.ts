...
describe('POC event logic', () => {
  it('scores Blink-Off time until a blink finishes the run', () => {
    ...
    event.update(3000, DEFAULT_INPUT);

    const frame = event.update(16, { ...DEFAULT_INPUT, bothEyesClosed: true });
  ...
  it('scores Face Weightlifting hold time when eyebrows lift the bar', () => {
    ...
    for (let i = 0; i < 80; i++) {
      event.update(100, { ...DEFAULT_INPUT, eyebrowsRaised: 0.9 });
    }
  ...
  it('scores Dragon Blast hits after charge and release', () => {
    ...
    event.start(DEFAULT_CALIBRATION);
    event.update(1500, { ...DEFAULT_INPUT, mouthOpen: 0.9 });
    event.update(100, { ...DEFAULT_INPUT, mouthOpen: 0 });
